...
export async function viewLeaderboard(game, channel, pointValue) {
  ...
  async function sendMessageForGroup(groupMessage) {
   ...
   for (let i = 0; i < playersArray.length; i += MAX_PLAYERS_PER_MESSAGE) {
    ...
    for (let j = i; j < i + MAX_PLAYERS_PER_MESSAGE && j < playersArray.length; j++) {
      ...
      if (player.liquidated){
        ...
        const win_rate = (player.num_correct/player.num_trades*100).toFixed(2);

        groupMessage += `${j + 1}. ${player.displayName} - Profit: ${form_profit}, Number of Trades: ${num_trades}, Win Rate: ${win_rate}% \n`;
      }
    }
...
export async function viewPositions(game, channel) {
  ...
  for (let i = 0; i < playersEntries.length; i += MAX_PLAYERS_PER_MESSAGE) {
    ...
    for (let j = i; j < i + MAX_PLAYERS_PER_MESSAGE && j < playersEntries.length; j++) {
      ...
      if (playerData.position) {
        ...
        if (
          (playerData.enterPrice < current_price && playerData.position == "long") ||
          (playerData.enterPrice > current_price && playerData.position == "short")
        ) {
          groupMessage += `🟢\n`;
        } else if (
          (playerData.enterPrice > current_price && playerData.position == "long") ||
...
export function calculateAllProfit(game, displayName, addProfit) {
  ...
  if (game.players[displayName].position === "long") {
    profit =
      game.stockPrice -
      game.players[displayName].enterPrice;
